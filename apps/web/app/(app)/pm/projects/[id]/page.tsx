...
export default function PMProjectDetailPage() {
    ...
    async function loadProject() {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(`[Project Page] Failed to load project: status=${response.status}, error=`, errorData);
  ...
  const handleExpenseSave = async () => {
    ...
    try {
      ...
      // Проверяем, была ли трата автоматически переведена в pending
  ...
  const handleMarketplacePublish = async () => {
    ...
    try {
      ...
      // Открываем страницу листинга в маркетплейсе
      if (data.listing?.id) {
        router.push(`/market/projects/${data.listing.id}`);
      }
    } catch (err) {
  ...
  if (error || !project) {
    // Не делаем редирект автоматически, показываем ошибку
    return (
      <div className="rounded-3xl border border-rose-500/40 bg-rose-500/10 p-6 text-sm text-rose-100">
        ...
        <p className="mt-2">{error || 'Проект не найден'}</p>
        <p className="mt-2 text-xs opacity-75">ID проекта: {projectId}</p>
        <button
          type="button"
          onClick={() => router.push('/pm/projects')}
          className="mt-4 rounded-lg border border-rose-500/40 bg-rose-500/20 px-4 py-2 text-sm font-medium transition hover:bg-rose-500/30"
        >
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Модальное окно настроек бюджета */}
      {flags.BUDGET_LIMITS && (
        <BudgetSettingsModal
          projectId={projectId}
          workspaceId={project.workspaceId || 'current'}
