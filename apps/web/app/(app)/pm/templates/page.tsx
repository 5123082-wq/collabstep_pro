...
import { Plus, Edit, Trash2, RefreshCw, FileText, Palette, Globe, Megaphone, Sparkles, type LucideIcon } from 'lucide-react';
...
function getKindColor(kind: string | undefined): string {
...
export default function PMTemplatesPage() {
  ...
  const loadTemplates = useCallback(async () => {
    ...
    try {
      ...
      const data = (await response.json()) as { items: UserTemplate[] };
      setTemplates(data.items);
    } catch (err) {
      console.error('[PMTemplatesPage] Ошибка загрузки:', err);
  ...
  const handleSubmit = async () => {
    if (!formData.title.trim()) {
      toast('Заполните название шаблона', 'warning');
      return;
    }
    ...
    try {
      const url = editingTemplate
        ? `/api/pm/templates/${editingTemplate.id}`
        : '/api/pm/templates';
      ...
      const data = (await response.json()) as { item: UserTemplate };
      if (editingTemplate) {
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1.5">
          ...
          <div className="space-y-1">
            <h1 className="text-xl font-semibold text-white sm:text-2xl">Мои шаблоны</h1>
            <p className="text-sm text-neutral-400">
              Создавайте и управляйте своими шаблонами проектов для быстрого старта
            </p>
          </div>
        </div>
        ...
        <>
          {templates.length === 0 ? (
            <ContentBlock variant="dashed" className="text-center">
              ...
              <p className="mt-4 text-sm text-neutral-400">У вас пока нет шаблонов</p>
              <button
                onClick={handleCreate}
      ...
      <Modal open={isModalOpen} onOpenChange={setIsModalOpen}>
        <ModalContent className="max-w-xl">
          <ModalHeader>
            ...
            <ModalDescription>
              {editingTemplate
                ? 'Измените данные шаблона проекта.'
                : 'Создайте новый шаблон проекта для быстрого старта.'}
            </ModalDescription>
            <ModalClose />
          </ModalHeader>
          ...
          <ModalBody className="space-y-4">
            <div className="space-y-2">
              <label htmlFor="template-title" className="text-sm font-medium text-neutral-200">
                Название <span className="text-neutral-500">*</span>
              </label>
              <input
                id="template-title"
                type="text"
            ...
            <div className="space-y-2">
              <label htmlFor="template-summary" className="text-sm font-medium text-neutral-200">
                Описание <span className="text-neutral-500">*</span>
              </label>
              <textarea
                id="template-summary"
