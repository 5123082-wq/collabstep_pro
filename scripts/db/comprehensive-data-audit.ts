...
// Log environment status before imports
console.log('[Script] Environment status:', {
  POSTGRES_URL: process.env.POSTGRES_URL ? 'set' : 'not set',
  DATABASE_URL: process.env.DATABASE_URL ? 'set' : 'not set',
  AUTH_STORAGE: process.env.AUTH_STORAGE || 'not set',
...
async function auditData() {
  try {
    ...
    console.log('📋 Конфигурация репозиториев:');
    console.log(`   AUTH_STORAGE: ${process.env.AUTH_STORAGE || 'не установлен (по умолчанию: memory)'}`);
    console.log(`   POSTGRES_URL: ${process.env.POSTGRES_URL ? 'установлен' : 'не установлен'}`);
    console.log(`   DATABASE_URL: ${process.env.DATABASE_URL ? 'установлен' : 'не установлен'}`);
    console.log(`   OrganizationsRepository: ${organizationsRepository.constructor.name}`);
    console.log(`   ProjectsRepository: ${projectsRepository.constructor.name}`);
    console.log(`   TasksRepository: ${tasksRepository.constructor.name}\n`);
    ...
    const orgsFromRepo = user ? await organizationsRepository.listForUser(user.id) : [];
    ...
    orgsFromSql.forEach((org: Record<string, unknown>) => {
      const key = org.id;
      if (!allOrgsMap.has(key)) {
        allOrgsMap.set(key, {
          id: org.id,
    ...
    // Репозиторий
    const projectsFromRepo = await projectsRepository.list();
    ...
    projectsFromSql.forEach((project: Record<string, unknown>) => {
      const key = project.id;
      if (!allProjectsMap.has(key)) {
        allProjectsMap.set(key, {
          id: project.id,
    ...
    // Репозиторий
    const tasksFromRepo = await tasksRepository.list();
    ...
    tasksFromSql.forEach((task: Record<string, unknown>) => {
      const key = task.id;
