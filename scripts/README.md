...
Быстрая выгрузка изменений на GitHub и создание Pull Request.

Автоматически выполняет:
- Проверку статуса git репозитория
- Добавление всех изменений
- Создание коммита с описанием
- Создание новой ветки (если находитесь на базовой ветке)
...
```bash
# Быстрая команда (после source ~/.zshrc или перезапуска терминала)
pr "Добавлена функция авторизации"
/PR "Добавлена функция авторизации"  # Алиас

# Git alias
git pr "Добавлена функция авторизации"

# NPM команда
pnpm pr "Добавлена функция авторизации"

# Прямой вызов скрипта
bash scripts/utils/create-pr.sh "Описание изменений"

# Без аргумента (скрипт запросит описание)
pr
```

**Настройка быстрой команды:**

Функция `pr` и алиас `/PR` автоматически добавлены в `~/.zshrc`. После первого использования выполните:
...
pr "Добавлена валидация форм через Zod"
pr "Обновлена документация модуля Performers"
```
