...
### Дополнительные технические правила для AI

- **Учитывать `exactOptionalPropertyTypes`:** не присваивать `undefined` опциональным полям. Использовать условные спреды или `omitUndefined`.
- **Входные данные из Zod:** удалять `undefined` перед передачей в сервисы/репозитории.
- **Типы и дерево-структуры:** избегать `unknown[]`, использовать явные типы/guards для `children`.
- **`@ts-expect-error`:** применять только с явным комментарием причины (исключение: иконки `lucide-react`).
- **После изменений TS:** запускать `pnpm -w typecheck` или явно писать, что не запускалось.
