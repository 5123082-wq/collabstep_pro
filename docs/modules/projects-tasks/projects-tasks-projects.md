...
**Источники:**
- Шаблоны проектов и каталог решений: [`../marketplace/marketplace-overview.md`](../marketplace/marketplace-overview.md)
...
- **`admin`** — администратор проекта. Управление задачами и командой
- **`member`** — участник. Создание и редактирование задач
- **`viewer`** — наблюдатель. Только просмотр

**Новый базовый flow people picker:**
1. PM surface показывает поиск по зарегистрированным пользователям платформы
2. Для каждого кандидата видно состояние: уже в проекте / в команде организации / только на платформе
3. Existing user можно добавить напрямую в проект ролью `admin`, `member` или `viewer`
4. Invite-link остаётся fallback для случаев, когда человека ещё нет на платформе или доступ нужно отправить вручную
...
2. **Через people picker** — поиск и прямое добавление зарегистрированного пользователя в проект
3. **По email / внешнему каналу** — invite-link остаётся fallback

**Процесс:**
- Если пользователь уже зарегистрирован → direct add в проект из people picker
- Если пользователь не зарегистрирован → invite-link / регистрационный flow
- Назначение в задачи остаётся project-scoped и использует только команду проекта
...
- Быстрые действия (создать задачу, пригласить участника, создать трату, опубликовать в Каталог)
...
Проект может быть опубликован в `Каталог` как публичное решение или reusable starting point.
...
2. Заполняет публичную карточку решения
3. Настраивает описание, теги, CTA и условия доступа
4. Публикует объект в каталог

**См. также:**
- [`../marketplace/marketplace-overview.md`](../marketplace/marketplace-overview.md) — документация по каталогу
