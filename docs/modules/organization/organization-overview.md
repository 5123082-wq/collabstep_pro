...
- Закрытие организации описано в архиве: `docs/archive/2025-01-15-organization-closure/*`.
- 🆕 **Мульти-аккаунты (Multi-Org):** Пользователи могут иметь несколько организаций (переключение через `OrganizationSwitcher`). Первая созданная организация помечается флагом `is_primary`.
- 🆕 **Подписки:** Работает интеграция лимитов организаций через планы (Free, Pro, Max). Внедрен пэйволл (`UpgradeToCreateOrgModal`).
...
- **personal** — персональная организация пользователя (имя = имя пользователя). При создании первой организации она получает статус `is_primary`.
- **business** — организация с названием и командой

### Лимиты и подписки

- У пользователя всегда есть персональная организация (создаётся при регистрации; при invite‑регистрации — всегда personal).
- Лимит создаваемых организаций зависит от подписки (таблица `user_subscription`): Free = 1 (жёсткий), Pro/Max = `unlimited`.
- Для пользователей Free тарифа при попытке создать вторую организацию показывается `UpgradeToCreateOrgModal`.
- Shared‑план: personal + первая business делят `user_subscription`; остальные business используют `organization_subscription`.
...
- [Архив: Закрытие организации](../../archive/2025-01-15-organization-closure/organization-closure-specification.md)
