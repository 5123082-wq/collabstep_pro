# Чеклист настройки Vercel Postgres

## ✅ Текущее состояние

После очистки базы данных и настройки системы:

- ✅ База данных очищена (только `admin.demo@collabverse.test`)
- ✅ Память очищена (только администратор)
- ✅ Логика входа обновлена (блокирует удаленных пользователей)
- ✅ Форма регистрации работает корректно
- ✅ API маршруты проверены

## 📋 Что нужно сделать для работы с Vercel Postgres

### 1. Создать базу данных Vercel Postgres

1. Откройте ваш проект на [vercel.com](https://vercel.com)
2. Перейдите в раздел **Storage**
3. Нажмите **Create Database**
4. Выберите **Postgres**
5. Выберите регион (рекомендуется тот же, что и для деплоя)
6. Нажмите **Create**

Vercel автоматически создаст переменные окружения:
- `POSTGRES_URL` ✅
- `POSTGRES_PRISMA_URL`
- `POSTGRES_URL_NON_POOLING`
- `POSTGRES_USER`
- `POSTGRES_HOST`
- `POSTGRES_PASSWORD`
- `POSTGRES_DATABASE`

### 2. Настроить переменные окружения на Vercel

В настройках проекта на Vercel (Settings → Environment Variables) добавьте:

#### Обязательные переменные:

```env
# Database (используйте POSTGRES_URL, созданный автоматически)
DATABASE_URL=$POSTGRES_URL

# Auth Storage (включает использование БД)
AUTH_STORAGE=db

# NextAuth
AUTH_SECRET=<сгенерируйте: openssl rand -base64 32>
NEXTAUTH_URL=https://your-domain.vercel.app

# Google OAuth (если используете)
GOOGLE_CLIENT_ID=<ваш Google Client ID>
GOOGLE_CLIENT_SECRET=<ваш Google Client Secret>
```

#### Рекомендуемые переменные:

```env
# Базовые настройки
NAV_V1=on
APP_LOCALE=ru
FEATURE_PROJECTS_V1=1
AUTH_DEV=on

# Finance Storage
FIN_EXPENSES_STORAGE=db

# Демо-администратор
DEMO_ADMIN_EMAIL=admin.demo@collabverse.test
DEMO_ADMIN_PASSWORD=admin.demo
```

> ⚠️ **Важно:** Удалите `DEMO_USER_EMAIL` и `DEMO_USER_PASSWORD`, так как этот пользователь больше не используется.

### 3. Настроить локальные переменные окружения

Создайте/обновите файл `apps/web/.env.local`:

```env
# Database
POSTGRES_URL=<ваш POSTGRES_URL из Vercel>
DATABASE_URL=$POSTGRES_URL

# Auth Storage
AUTH_STORAGE=db

# NextAuth
AUTH_SECRET=<сгенерируйте: openssl rand -base64 32>
NEXTAUTH_URL=http://localhost:3000

# Google OAuth (если используете)
GOOGLE_CLIENT_ID=<ваш Google Client ID>
GOOGLE_CLIENT_SECRET=<ваш Google Client Secret>

# Базовые настройки
NAV_V1=on
APP_LOCALE=ru
FEATURE_PROJECTS_V1=1
AUTH_DEV=on

# Finance Storage
FIN_EXPENSES_STORAGE=db

# Демо-администратор
DEMO_ADMIN_EMAIL=admin.demo@collabverse.test
DEMO_ADMIN_PASSWORD=admin.demo
```

### 4. Применить миграции базы данных

#### Вариант A: Через Vercel CLI (рекомендуется)

```bash
# Установите Vercel CLI (если еще не установлен)
npm i -g vercel

# Войдите в Vercel
vercel login

# Подключитесь к проекту
vercel link

# Получите переменные окружения
vercel env pull .env.local

# Примените миграции
cd apps/api
pnpm db:push
```

#### Вариант B: Через SQL редактор Vercel

1. Откройте вашу базу данных в Vercel
2. Перейдите в раздел **Query**
3. Выполните SQL из файлов миграций по порядку:
   - `apps/api/src/db/migrations/0000_lying_mauler.sql` (базовые таблицы: user, account, session, userControl)
   - `apps/api/src/db/migrations/0001_chemical_lake.sql` (организации, проекты, профили)
   - `apps/api/src/db/migrations/0002_spicy_domino.sql` (финансы: wallet, transaction, contract)

### 5. Проверить создание таблиц

В SQL редакторе Vercel выполните:

```sql
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public'
ORDER BY table_name;
```

Вы должны увидеть таблицы:
- `user` ✅
- `account` ✅
- `session` ✅
- `verificationToken` ✅
- `userControl` ✅
- `performer_profile` ✅
- `organization` ✅
- `organization_member` ✅
- `project` ✅
- `project_member` ✅
- `organization_invite` ✅
- `project_invite` ✅
- `wallet` ✅
- `transaction` ✅
- `contract` ✅

### 6. Инициализировать демо-администратора в БД

После применения миграций запустите скрипт очистки (он создаст администратора, если его нет):

```bash
npx tsx scripts/cleanup-users-db.ts
```

Или вручную через SQL:

```sql
-- Проверьте, что администратор существует
SELECT id, name, email, "passwordHash" IS NOT NULL as has_password
FROM "user"
WHERE email = 'admin.demo@collabverse.test';
```

### 7. Проверить работу системы

#### 7.1 Локально

```bash
# Запустите сервер
pnpm dev

# Откройте http://localhost:3000
# Попробуйте войти как admin.demo@collabverse.test
```

#### 7.2 На Vercel

1. Дождитесь успешного деплоя
2. Откройте ваше приложение
3. Попробуйте войти как `admin.demo@collabverse.test`
4. Проверьте, что данные сохраняются в БД

### 8. Проверить данные в БД

```sql
-- Проверка пользователей
SELECT id, name, email, "emailVerified", 
       "passwordHash" IS NOT NULL as has_password,
       "createdAt"
FROM "user"
ORDER BY "createdAt" DESC;

-- Проверка прав администратора
SELECT uc."userId", u.email, uc.status, uc.roles, uc."testerAccess"
FROM "userControl" uc
JOIN "user" u ON uc."userId" = u.id
WHERE u.email = 'admin.demo@collabverse.test';
```

## 🔧 Команды для работы с БД

### Генерация миграций

```bash
cd apps/api
pnpm drizzle-kit generate
```

### Применение миграций (push схемы)

```bash
cd apps/api
pnpm drizzle-kit push
```

### Просмотр схемы

```bash
cd apps/api
pnpm drizzle-kit studio
```

## ⚠️ Важные замечания

1. **POSTGRES_URL vs DATABASE_URL**
   - Vercel создает `POSTGRES_URL` автоматически
   - В коде используется `POSTGRES_URL` для подключения
   - `DATABASE_URL` можно установить равным `$POSTGRES_URL` для совместимости

2. **AUTH_STORAGE=db**
   - Обязательно установите `AUTH_STORAGE=db` для использования базы данных
   - Без этого система будет использовать память (memory)

3. **Миграции**
   - Миграции нужно применить только один раз
   - После применения миграций база данных готова к работе
   - Скрипт `cleanup-users-db.ts` создаст администратора автоматически

4. **Демо-пользователи**
   - Удалены все демо-пользователи кроме `admin.demo@collabverse.test`
   - Не устанавливайте `DEMO_USER_EMAIL` и `DEMO_USER_PASSWORD`
   - Новые пользователи регистрируются через форму `/register`

## 🐛 Устранение неполадок

### Ошибка подключения к БД

**Проблема:** `ECONNREFUSED` или `connection timeout`

**Решение:**
1. Проверьте, что `POSTGRES_URL` установлен корректно
2. Убедитесь, что используете `POSTGRES_URL` (с пулингом), а не `POSTGRES_URL_NON_POOLING`
3. Проверьте, что база данных активна в панели Vercel
4. Убедитесь, что `AUTH_STORAGE=db` установлен

### Таблицы не созданы

**Проблема:** Ошибки типа `relation "user" does not exist`

**Решение:**
1. Убедитесь, что миграции применены (см. Шаг 4)
2. Проверьте логи деплоя на наличие ошибок
3. Вручную выполните SQL из файлов миграций в SQL редакторе Vercel

### Пользователь не может войти

**Проблема:** Ошибка авторизации даже для администратора

**Решение:**
1. Проверьте, что администратор создан в БД:
   ```sql
   SELECT * FROM "user" WHERE email = 'admin.demo@collabverse.test';
   ```
2. Проверьте, что у администратора есть пароль:
   ```sql
   SELECT "passwordHash" IS NOT NULL FROM "user" WHERE email = 'admin.demo@collabverse.test';
   ```
3. Запустите скрипт очистки для создания/обновления администратора:
   ```bash
   npx tsx scripts/cleanup-users-db.ts
   ```

### Система все еще использует память

**Проблема:** Изменения не сохраняются в БД

**Решение:**
1. Проверьте, что `AUTH_STORAGE=db` установлен
2. Проверьте, что `POSTGRES_URL` установлен
3. Перезапустите сервер после изменения переменных окружения
4. Проверьте логи на наличие ошибок подключения к БД

## 📚 Дополнительные ресурсы

- [Vercel Postgres Documentation](https://vercel.com/docs/storage/vercel-postgres)
- [NextAuth.js Documentation](https://next-auth.js.org/)
- [Drizzle ORM Documentation](https://orm.drizzle.team/)
- [Подробное руководство по настройке Vercel Postgres](../docs/getting-started/vercel-postgres-setup.md)

## ✅ Финальная проверка

После выполнения всех шагов проверьте:

- [ ] База данных создана на Vercel
- [ ] Переменные окружения установлены на Vercel
- [ ] Локальный `.env.local` настроен
- [ ] Миграции применены
- [ ] Таблицы созданы в БД
- [ ] Администратор существует в БД
- [ ] Вход работает локально
- [ ] Вход работает на Vercel
- [ ] Форма регистрации работает
- [ ] Данные сохраняются в БД

После выполнения всех пунктов система готова к работе! 🎉

